...
ERL_NIF_TERM
pqclean_nif_mceliece6960119f_info_0(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
#define RET_MAP_SIZE (6)
    ...
    vals[v++] = enif_make_string(env, PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_ALGNAME, ERL_NIF_LATIN1);
    keys[k++] = enif_make_atom(env, "secretkeybytes");
    vals[v++] = enif_make_uint(env, PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_SECRETKEYBYTES);
    keys[k++] = enif_make_atom(env, "publickeybytes");
    vals[v++] = enif_make_uint(env, PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_PUBLICKEYBYTES);
    keys[k++] = enif_make_atom(env, "ciphertextbytes");
    vals[v++] = enif_make_uint(env, PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    keys[k++] = enif_make_atom(env, "sharedsecretbytes");
    vals[v++] = enif_make_uint(env, PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_BYTES);
...
ERL_NIF_TERM
pqclean_nif_mceliece6960119f_keypair_0(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    int retval = -1;
...

    {
        ...
        if (_pqclean_nif_get_thread_stack_size(&stack_size, NULL, NULL) != 0 || stack_size < 0x800000ULL) {
            return EXCP_BADARG_F(
                ...
                PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_ALGNAME, stack_size / 1024 / sizeof(uintptr_t));
        }
    };

    pk = enif_make_new_binary(env, PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_PUBLICKEYBYTES, &pk_term);
    if (pk == NULL) {
    ...
    sk = enif_make_new_binary(env, PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_SECRETKEYBYTES, &sk_term);
    if (sk == NULL) {
    ...
    retval = PQCLEAN_MCELIECE6960119F_CLEAN_crypto_kem_keypair(pk, sk);
    if (retval != 0) {
        return EXCP_BADARG(env, "Call to PQCLEAN_MCELIECE6960119F_CLEAN_crypto_kem_keypair() failed");
    }
    return enif_make_tuple2(env, pk_term, sk_term);
}

ERL_NIF_TERM
pqclean_nif_mceliece6960119f_encapsulate_1(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    int retval = -1;
    ErlNifBinary pk_bin;
    ...
    if (!enif_inspect_binary(env, argv[0], &pk_bin) || pk_bin.size != PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_PUBLICKEYBYTES) {
        return EXCP_BADARG_F(env, "PublicKey is invalid (must be a binary of size %d-bytes)",
                             PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_PUBLICKEYBYTES);
    }
    ct = enif_make_new_binary(env, PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_CIPHERTEXTBYTES, &ct_term);
    if (ct == NULL) {
    ...
    ss = enif_make_new_binary(env, PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_BYTES, &ss_term);
    if (ss == NULL) {
    ...
    retval = PQCLEAN_MCELIECE6960119F_CLEAN_crypto_kem_enc(ct, ss, pk_bin.data);
    if (retval != 0) {
        return EXCP_BADARG(env, "Call to PQCLEAN_MCELIECE6960119F_CLEAN_crypto_kem_enc() failed");
    }
    return enif_make_tuple2(env, ct_term, ss_term);
}

ERL_NIF_TERM
pqclean_nif_mceliece6960119f_decapsulate_2(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    int retval = -1;
    ErlNifBinary ct_bin;
    ErlNifBinary sk_bin;
    ...
    if (!enif_inspect_binary(env, argv[0], &ct_bin) || ct_bin.size != PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_CIPHERTEXTBYTES) {
        return EXCP_BADARG_F(env, "CipherText is invalid (must be a binary of size %d-bytes)",
                             PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_CIPHERTEXTBYTES);
    }
    if (!enif_inspect_binary(env, argv[1], &sk_bin) || sk_bin.size != PQCLEAN_MCELIECE6960119F_CLEAN_CRYPTO_SECRETKEYBYTES) {
    ...
    retval = PQCLEAN_MCELIECE6960119F_CLEAN_crypto_kem_dec(ss, ct_bin.data, sk_bin.data);
    if (retval != 0) {
        return EXCP_BADARG(env, "Call to PQCLEAN_MCELIECE6960119F_CLEAN_crypto_kem_dec() failed");
    }
    return ss_term;
}