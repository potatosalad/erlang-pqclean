...
ERL_NIF_TERM
pqclean_nif_falcon1024_info_0(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
#define RET_MAP_SIZE (5)
    ...
    vals[v++] = enif_make_string(env, PQCLEAN_FALCON1024_CLEAN_CRYPTO_ALGNAME, ERL_NIF_LATIN1);
    keys[k++] = enif_make_atom(env, "secretkeybytes");
    vals[v++] = enif_make_uint(env, PQCLEAN_FALCON1024_CLEAN_CRYPTO_SECRETKEYBYTES);
    keys[k++] = enif_make_atom(env, "publickeybytes");
    vals[v++] = enif_make_uint(env, PQCLEAN_FALCON1024_CLEAN_CRYPTO_PUBLICKEYBYTES);
    keys[k++] = enif_make_atom(env, "signaturebytes");
    vals[v++] = enif_make_uint(env, PQCLEAN_FALCON1024_CLEAN_CRYPTO_BYTES);
...
ERL_NIF_TERM
pqclean_nif_falcon1024_keypair_0(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    int retval = -1;
    ...
    pk = enif_make_new_binary(env, PQCLEAN_FALCON1024_CLEAN_CRYPTO_PUBLICKEYBYTES, &pk_term);
    if (pk == NULL) {
    ...
    sk = enif_make_new_binary(env, PQCLEAN_FALCON1024_CLEAN_CRYPTO_SECRETKEYBYTES, &sk_term);
    if (sk == NULL) {
    ...
    retval = PQCLEAN_FALCON1024_CLEAN_crypto_sign_keypair(pk, sk);
    if (retval != 0) {
        return EXCP_ERROR(env, "Call to PQCLEAN_FALCON1024_CLEAN_crypto_sign_keypair() failed");
    }
    return enif_make_tuple2(env, pk_term, sk_term);
}

ERL_NIF_TERM
pqclean_nif_falcon1024_sign_2(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    int retval = -1;
    ErlNifBinary m_bin;
    ErlNifBinary sk_bin;
    ...
    if (!enif_inspect_binary(env, argv[1], &sk_bin) || sk_bin.size != PQCLEAN_FALCON1024_CLEAN_CRYPTO_SECRETKEYBYTES) {
        return EXCP_BADARG_F(env, "SecretKey is invalid (must be a binary of size %d-bytes)",
                             PQCLEAN_FALCON1024_CLEAN_CRYPTO_SECRETKEYBYTES);
    }
    sig = enif_make_new_binary(env, PQCLEAN_FALCON1024_CLEAN_CRYPTO_BYTES, &sig_term);
    if (sig == NULL) {
    ...
    retval = PQCLEAN_FALCON1024_CLEAN_crypto_sign_signature(sig, &siglen, m_bin.data, m_bin.size, sk_bin.data);
    if (retval != 0) {
        return EXCP_ERROR(env, "Call to PQCLEAN_FALCON1024_CLEAN_crypto_sign_signature() failed");
    }
    if (siglen < PQCLEAN_FALCON1024_CLEAN_CRYPTO_BYTES) {
...
ERL_NIF_TERM
pqclean_nif_falcon1024_verify_3(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    int retval = -1;
    ...
    if (!enif_inspect_binary(env, argv[0], &sig_bin) || sig_bin.size > PQCLEAN_FALCON1024_CLEAN_CRYPTO_BYTES) {
    ...
    if (!enif_inspect_binary(env, argv[2], &pk_bin) || pk_bin.size != PQCLEAN_FALCON1024_CLEAN_CRYPTO_PUBLICKEYBYTES) {
        return EXCP_BADARG_F(env, "PublicKey is invalid (must be a binary of size %d-bytes)",
                             PQCLEAN_FALCON1024_CLEAN_CRYPTO_PUBLICKEYBYTES);
    }

    retval = PQCLEAN_FALCON1024_CLEAN_crypto_sign_verify(sig_bin.data, sig_bin.size, m_bin.data, m_bin.size, pk_bin.data);
    if (retval != 0) {
        return ATOM(false);
    }
    return ATOM(true);
